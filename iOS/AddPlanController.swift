...
class AddPlanController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UITextFieldDelegate {
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if SaveButton === sender {
            ...
            newPlan = Plan(roleName: self.RoleNameField.text!, anime: self.AnimeField.text!, version: self.VersionField.text!, photo: self.PhotoImageView.image, ware: [:], stuff: [:], date: dateInput(Int(self.YearField.text!)!, month: Int(self.MonthField.text!)!, day: Int(self.DayField.text!)!))
        }
    }*/
        ...
        if checkDateValid(){
            ...
            if thisDate.compare(NSDate()) == NSComparisonResult.OrderedAscending{
                ...
                if SaveButton === sender {
                    ...
                    newPlan = Plan(roleName: self.RoleNameField.text!, anime: self.AnimeField.text!, version: self.VersionField.text!, photo: self.PhotoImageView.image, ware: [:], stuff: [:], date: thisDate)
                }
                self.performSegueWithIdentifier("unwindToPlanList", sender: self)
            }
        }else{
            if localization == "zh-HK" || localization == "zh-Hant-MO" || localization == "zh-TW"{
